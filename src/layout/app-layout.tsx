...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<IndividualInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/individuals` : null),
    () => getIndividuals({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Individual',
      path: isTenantUser ? `/individuals/view/${data?.[0]?.id}` : '/individuals',
      entity: 'individual',
    ...
    { name: 'Task', path: '/tasks', entity: 'task', service: AccessServiceEnum.PROJECT, icon: FiClock },
