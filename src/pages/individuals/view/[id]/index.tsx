...
import { getIndividualById } from 'apiSdk/individuals';
...
function IndividualViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<IndividualInterface>(
    () => (id ? `/individuals/${id}` : null),
    () =>
      getIndividualById(id, {
        relations: ['user', 'performance_evaluation', 'task'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('individual', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/individuals/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('performance_evaluation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/performance-evaluations/create?individual_id=${data?.id}`}>
              ...
              {hasAccess('task', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/tasks/create?individual_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
