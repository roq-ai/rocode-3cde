...
import { getIndividualById, updateIndividualById } from 'apiSdk/individuals';
...
function IndividualEditPage() {
  ...
  const handleSubmit = async (values: IndividualInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateIndividualById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<IndividualInterface>({
    initialValues: data,
